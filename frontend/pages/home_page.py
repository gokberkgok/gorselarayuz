...
class HomePage(QWidget):
    ...
    def _build_ui(self):
        ...
        welcome = QLabel(f"👋  Welcome back, {self.api.user_name or 'User'}!")
        welcome.setStyleSheet("font-size: 28px; font-weight: 700; color: white; background: transparent;")
        banner_layout.addWidget(welcome)

        welcome_sub = QLabel("Discover and reserve amazing places today")
        welcome_sub.setStyleSheet("font-size: 15px; color: rgba(255,255,255,0.85); background: transparent;")
        banner_layout.addWidget(welcome_sub)
        layout.addWidget(banner)

        # Section title
        section_row = QHBoxLayout()
        sec_title = QLabel("🌟  Featured Places")
    ...
    def load_data(self):
        ...
        try:
            ...
            err = QLabel(f"❌ Failed to load places: {e}")
